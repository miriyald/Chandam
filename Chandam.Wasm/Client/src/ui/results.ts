...
export async function renderResults(matches: ChandamMatch[], containerId: string, ruleSet?: string) {
  ...
  container.innerHTML = matches.map(match => renderMatchCard(match, ruleSet)).join('') + renderDisclaimer();
...
export async function renderFirstMatch(match: ChandamMatch, containerId: string, ruleSet?: string, options?: { showRuleLink?: boolean }) {
  ...
  container.innerHTML = renderMatchCard(match, ruleSet, showLink) + renderDisclaimer();
  attachResultActionHandlers(container, ruleSet);
...
function renderMatchCard(match: ChandamMatch, ruleSet?: string, showRuleLink = true): string {
  ...
  const isGenericVruttam = match.rule.identifier === 'GenericVruttam';

  // Generate rule details link (opens in new tab) — hidden on rule compute pages
  const ruleLink = (showRuleLink && ruleSet && match.rule.identifier && !isGenericVruttam)
    ? `<a href="${makeUrl(`/learn/${ruleSet}/${match.rule.identifier}/`)}" class="action-btn rule-details-link" target="_blank" rel="noopener noreferrer"><span class="material-symbols-outlined" style="font-size:16px" aria-hidden="true">menu_book</span> <span>${t('results_view_details')}</span></a>`
    : '';
  ...
  const addExampleBtn = (match.matchPercentage >= 95 && isCustomRule)
    ? `<button class="action-btn btn-add-example" data-rule-id="${escapeAttr(match.rule.identifier)}" title="${t('results_add_to_examples')}"><span class="material-symbols-outlined" style="font-size:16px" aria-hidden="true">add</span> <span>${t('results_add_to_examples')}</span></button>`
    : '';
  const submitGithubBtn = (match.matchPercentage >= 95 && ruleSet && !isGenericVruttam)
  ...
  const addToCollectionBtn = (match.matchPercentage >= 95 && ruleSet && !isGenericVruttam)
...
function renderDisclaimer(): string {
  return `<div class="results-disclaimer">${t('results_disclaimer')}</div>`;
}
