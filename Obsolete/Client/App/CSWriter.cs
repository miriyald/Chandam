...
{ 
	...
	{ 
		...
		Examples = {15};
			...
			string identifier = C.RTS(R.Name);
			string nm = R.Name.Replace(identifier, "").Trim();
		...
		internal string CSRules(object[][] p, RuleType rt)
		{
			...
			{
				case RuleType.Name:
					return CStringArrayArray((string[][])p);
				case RuleType.Type:
					return CSTypeArrayArray(p);
				case RuleType.Type2:
					return CSType2ArrayArray(p);
				case RuleType.Weight:
		...
		private string CSType2ArrayArray(object[][] p)
		{
			string s = "new object[][]{\n";
			foreach (object[] row in p)
			{
		...
		private string CStringArrayArray(string[][] p)
		{
			string s = "new string[][]{";
			foreach (string[] row in p)
			{
				s = s + "new string[]{";
