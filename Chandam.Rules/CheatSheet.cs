//---------------------------------------------------------------------------------------------
// <copyright file="CheatSheet.cs" company="Chandam-ఛందం">
	...
	public partial class CheatSheet
	{
		...
		public static string BuildCheatSheet2(bool extended, bool fixedWidth, Rule[] Rules)
		{
			...
			foreach (Rule R in Rules)
			{
				...
				if (extended)
				{
					row = row + "<td>" + (R.Examples2 == null ? "" : R.Examples2.Length.ToString()) + "</td>";
				...
				case RuleType.Type:
					{
						...
						return GDefinition.CategoryString(Cat);
					}
				case RuleType.SubType:
					{
						SubCategory Cat = (SubCategory)o;
						return GDefinition.SubCategoryString(Cat);
					}
		...
		public static string BuildHTMLRules(Rule R, bool includeExamples)
		{
			...
			if (includeExamples)
			{
				...
				if (R.Examples2.Length == 0)
				{
					s = s + " అందుబాటులో లేవు. ";
				}

				s = s + "<ol>";
				foreach (Example ex in R.Examples2)
				{
					//s = s + "<li>" +   ex.Replace ( "\n" , "<br/>" ) + "</li>";
					s = s + "<li>" + TryMatch(ex.Text, R) + "</li>";
				}
				s = s + "</ol></li>";
			}
			s = s + "</ol>";
